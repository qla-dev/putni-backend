...
use App\Http\Resources\LampyrisUserResource;
...
class LampyrisAuthController extends Controller
{
    ...
    {
        return response()->json(['user' => new LampyrisUserResource($request->user())]);
    }
    ...
    private function authenticate(string $provider, mixed $providerId, mixed $email, mixed $name)
    {
        ...
        $query = LampyrisUser::query()->where($providerField, $providerId);
        ...
        return response()->json([
            ...
            'user' => new LampyrisUserResource($user->refresh()),
    ...
    {
        $clientIds = config('services.lampyris.google_client_ids', []);
        if ($clientIds === []) {
    ...
    {
        $clientIds = config('services.lampyris.apple_client_ids', []);
